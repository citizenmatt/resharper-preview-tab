  ...
  <xsl:template match="/PluginLocalInfo">
    <UpdateInfos>
      ...
      <xsl:if test="($InstalledMajor &lt; $LatestMajor) or ($InstalledMajor = $LatestMajor and $InstalledMinor &lt; $LatestMinor) or ($InstalledMajor = $LatestMajor and $InstalledMinor = $LatestMinor and $InstalledBuild &lt; $LatestBuild)">
        ...
        <UpdateInfo>
          <InformationUri>https://github.com/citizenmatt/resharper-preview-tab/wiki/Release-Notes</InformationUri>
          <Title>
            <xsl:value-of select="concat('Preview Tab Behaviour ', $LatestMajor, '.', $LatestMinor, '.', $LatestBuild, ' Released')" />
          </Title>
          <Description>A minor upgrade is available.</Description>
          <DownloadUri>
            <xsl:value-of select="concat('http://download.jetbrains.com/resharper/plugins/resharper-preview-tab.', $LatestMajor, '.', $LatestMinor, '.zip')" />
          </DownloadUri>
